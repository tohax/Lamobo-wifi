...
void setled_record_start( int index )
{
	...
	{
		system("/etc/init.d/wifi_led.sh r_led off");
		system("/etc/init.d/wifi_led.sh r_led on");
	}
	else
	{
		system("/etc/init.d/wifi_led.sh r_led blink 250 250 ");
	}
...
void setled_record_stop( int index )
{
	...
	{
		system("/etc/init.d/wifi_led.sh r_led blink 1000 1000 ");
	}
	record_flag = 0;
	video_process_SetRecordFlag(record_flag);
}
